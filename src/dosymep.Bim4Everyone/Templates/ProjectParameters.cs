...
public class ProjectParameters {
    ...
    private void RevitParamsSync(Document source, Document target, IEnumerable<RevitParam> revitParams) {
        foreach(RevitParam revitParam in revitParams) {
            try {
                ...
                _loggerService.Warning(ex, "Не удалось синхронизировать параметр {@revitParam} в документе {@document}",
                    revitParam, target.Title);
            }
        }
    }
    ...
    private static void RevitParamBindingsSync(Document source, Document target, RevitParam revitParam) {
        if(source.IsExistsParam(revitParam)) {
            ...
            CategorySet targetCategories = ((ElementBinding) targetSettings.Binding).Categories;
            CategorySet sourceCategories = ((ElementBinding) sourceSettings.Binding).Categories;
            ...
            InternalDefinition sourceDefinition = (InternalDefinition) sourceSettings.Definition;
            InternalDefinition targetDefinition = (InternalDefinition) targetSettings.Definition;
    ...
    private static void CopyBrowserOrganization(Document target, Document document) {
        IList<Element> elements =
            new FilteredElementCollector(document).OfClass(typeof(BrowserOrganization)).ToElements();

        // Получаем только те элементы,
        // у которых совпадает имя с копируемыми настройками диспетчера видов
        ElementId[] removingElements = new FilteredElementCollector(target)
        ...
        ElementTransformUtils.CopyElements(document, elements.Select(item => item.Id).ToArray(), target,
    ...
    public void SetupRevitParam(Document target, RevitParam revitParam) {
        ...
        SetupRevitParams(target, revitParam);
    }
    ...
    /// <param name="replaceSchedule">true - если требуется заменить спецификацию, иначе false.</param>
    /// <param name="revitScheduleRule">Правила спецификации.</param>
    public bool SetupSchedules(Document target, bool replaceSchedule,
        IEnumerable<RevitScheduleRule> revitScheduleRule) {
        if(target is null) {
            throw new ArgumentNullException(nameof(target));
        }
    ...
    public void SetupBrowserOrganization(Document target) {
        if(Application == null) {
            throw new InvalidOperationException(
                $"Перед настройкой диспетчера видов нужно инициализировать свойство \"{nameof(Application)}\".");
        }
    ...
    private static IEnumerable<ViewSchedule> GetViewSchedules(Document document,
        IEnumerable<KeyScheduleRule> keyScheduleRules) {
    ...
    private static bool CopyViewSchedule(Document source, Document target, bool removeSchedule,
        ViewSchedule viewSchedule) {
        if(viewSchedule == null) {
            return false;
        }

        List<string> targetViewSchedules = new FilteredElementCollector(target)
