...
public class EmptyProject {
    ...
    public Document CreateEmptyProject(string documentPath) {
        ...
        if(Application == null) {
            throw new InvalidOperationException(
                $"Перед созданием проекта нужно инициализировать свойство \"{nameof(Application)}\".");
        }
    ...
    public Document CreateEmptyProject(string documentPath, SaveAsOptions saveAsOptions) {
        ...
        Document document = Application.NewProjectDocument(ModuleEnvironment.EmptyTemplatePath);
        try {
