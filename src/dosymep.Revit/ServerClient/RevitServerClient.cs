...
internal class RevitServerClient : IRevitServerClient {
    ...
    /// <inheritdoc />
    public string ServerName => _revitHttpClient.ServerName;

    /// <inheritdoc />
    public string ServerVersion => _revitHttpClient.ServerVersion;

    /// <inheritdoc />
    public async Task<ServerInformations> GetServerPropertiesAsync(CancellationToken cancellationToken = default) {
        HttpResponseMessage response = await _revitHttpClient.Get("serverProperties", cancellationToken);
    ...
    /// <inheritdoc />
    public async Task<RevitContents>
        GetContentsAsync(string folderPath, CancellationToken cancellationToken = default) {
        ...
        HttpResponseMessage response = await _revitHttpClient.Get(folderPath + "/contents", cancellationToken);
        string result = await response.Content.ReadAsStringAsync();
    ...
    /// <inheritdoc />
    public async Task<List<RevitContents>> GetAllRevitContentsAsync(string parentPath,
    ...
    public async Task<DirectoryInfo> GetDirectoryInfoAsync(string path, CancellationToken cancellationToken = default) {
        ...
        HttpResponseMessage response = await _revitHttpClient.Get(path + "/DirectoryInfo", cancellationToken);
        string result = await response.Content.ReadAsStringAsync();
    ...
    private async Task<List<RevitContents>> GetRevitContentsInternalAsync(RevitContents parentContents,
        ...
        foreach(RevitFolder revitFolder in parentContents.Folders) {
            ...
            RevitContents revitContents =
                await GetContentsAsync(parentContents.Path + "|" + revitFolder.Name, cancellationToken);
            revitContentsList.Add(revitContents);

            List<RevitContents> childrenContents =
                await GetRevitContentsInternalAsync(revitContents, cancellationToken);
